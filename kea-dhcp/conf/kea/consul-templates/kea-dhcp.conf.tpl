...
"Dhcp4": {
    ...
    //},
    "lease-database": {
    	"type": "{{ keyOrDefault "/dhcp4/lease-database/type" "postgres" }}",
    	"name": "{{ keyOrDefault "/dhcp4/lease-database/name" "postgres" }}",
    	"user": "{{ keyOrDefault "/dhcp4/lease-database/user" "postgres" }}",
    	"password": "{{ keyOrDefault "/dhcp4/lease-database/password" "postgres123" }}",
    	"host": "{{ keyOrDefault "/dhcp4/lease-database/host" "127.0.0.1" }}",
    	"port": {{ keyOrDefault "/dhcp4/lease-database/port" "5432" }}
    },
    "hosts-database": {
    	"type": "{{ keyOrDefault "/dhcp4/hosts-database/type" "postgres" }}",
    	"name": "{{ keyOrDefault "/dhcp4/hosts-database/name" "postgres" }}",
    	"user": "{{ keyOrDefault "/dhcp4/hosts-database/user" "postgres" }}",
    	"password": "{{ keyOrDefault "/dhcp4/hosts-database/password" "postgres123" }}",
    	"host": "{{ keyOrDefault "/dhcp4/hosts-database/host" "127.0.0.1" }}",
    	"port": {{ keyOrDefault "/dhcp4/hosts-database/port" "5432" }}
    },
    ...
    //      "library": "/usr/local/var/kea/libdhcp_flex_id.so",
    //      "parameters": {
    ...
    "subnet4": [
        {
            ...
            "pools": [ { "pool": "{{ key "/dhcp4/subent/pool" }}" } ],
  ...
  "loggers": [
    {
        ...
        "output_options": [
            {
                ...
                "output": "{{ keyOrDefault "/dhcp4/logger/output" "stdout" }}"
        ...
        "severity": "{{ keyOrDefault "/dhcp4/logger/severity" "INFO" }}",

        // If DEBUG level is specified, this value is used. 0 is least verbose,
        // 99 is most verbose. Be cautious, Kea can generate lots and lots
        // of logs if told to do so.
        "debuglevel": {{ keyOrDefault "/dhcp4/logger/debuglevel" "0" }}
    }
  ]
}
}
